...
# the following code is just for note. not for the current analysis
# moderated mediation analysis

# standize numberic values
aversivesound_social_relevant$groupsimilarity <- aversivesound_social_relevant$dummygroup * aversivesound_social_relevant$similarity
aversivesound_social_relevant$scalevalue <- scale(aversivesound_social_relevant$value)

# one: use a full model to test whether there is a moderation effect

model20 <- 

'scalevalue ~ a*postanxiety + b*dummygroup + c*anxietygroup + d*totalcommoninverse + 
f*similaritygroup
totalcommoninverse ~ g*postanxiety + h*dummygroup + i*anxietygroup
# indirect effect 
ingroup := (g*d)
outgroup := (g+i) * (d+f)
'

fit1 <- sem(model20, data = aversivesound_social_relevant)
inspect(fit1)
summary(fit1, fit.measures = TRUE) 

library(semPlot)
semPaths(fit1, mar= c (40,20,40,20))
semPaths(fit1,'est',mar=c(10,5,10,5),fade = FALSE,curveAdjacent = TRUE, style = "lisrel")

# second way: focue the model the first path x--->mediator

# test whether a1 and  a3, (and b) is singificant or not, if it is significant, then the mediation effect is moderated.
model21 <- '
value ~ c*anxiety  +  b*similarity
similarity ~ a1*anxiety + a2*dummygroup + a3*anxietygroup
# indirect effect (a*d) when group = 0(ingroup)
ingroup :=  a1 * b
# indirect effect (a*d) when group = 1(outgroup)
outgroup := (a1 + a3) * b
...
fit2 <- sem(model21, data = total.aversive.relevant)
inspect(fit2)
summary(fit2, fit.measures = TRUE) 

# plot
p.alldata <- ggplot(total_sound_aloneingroup, aes(x = valence, y = value, fill = group)) +
  geom_boxplot(alpha=0.7) +
  scale_y_continuous(name = "SCRs",
                     breaks = seq(-1.2, 1.2,0.4),
                     limits=c(-1.2,1.2)) +
  scale_x_discrete(name = "valence") +
  ggtitle("Boxplot of  EDA  by group") +
  theme_bw() +
